...
var ALERT_MULTIPLE, MAX_VALUE, NUM_OF_LAST_DURATIONS, NUM_OF_MOST_EXTREME, NUM_OF_REQUESTS, REQUEST_TYPES, TIMEOUT, addDuration, asyncLib, medianLib, requestLib, runChecks, urlLib, urlencodedParserLib, validUrlLib;
    ...
    return collection.find({}, {}, function(e, durations) {
      ...
      return asyncLib.series(durations.map(function(duration) {
        return function(callbackOuter) {
          var data, i, j, options, requestType, results1, results2, url, url_parts;
          url = duration['url'];
          options = {
            uri: url,
            time: true,
          ...
          if (requestType === 'GET') {
              ...
              return function(callbackInner) {
                return requestLib(options, function(error, response, body) {
                  ...
                  if (requestDurations.length === NUM_OF_REQUESTS) {
                    addDuration(db, options['uri'], requestDurations, timestamp);
                    requestDurations = [];
                  }
            ...
            return asyncLib.series((function() {
              ...
              return function(callbackInner) {
                return requestLib.post(options, data, function(error, response, body) {
                  requestDurations.push(error ? null : response.elapsedTime);
                  if (requestDurations.length === NUM_OF_REQUESTS) {
                    addDuration(db, options['uri'], requestDurations, timestamp);
                    requestDurations = [];
